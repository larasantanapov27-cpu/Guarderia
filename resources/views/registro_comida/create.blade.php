        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white">
            <div class="card-body p-4">
                ...
                <div class="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom border-light">
                    ...
                    <div>
                        <h5 class="fw-bold text-dark mb-0" style="letter-spacing: -0.5px;">Registrar Consumo de Alimento</h5>
                        <small class="text-muted">Añade un nuevo registro diario al historial de alimentación</small>
                    </div>
                </div>

                <form action="{{ route('registro_comidas.store') }}" method="POST" id="formCreateRegistroComida">
                    @csrf
                    
                    {{-- Select: Seleccionar Niño (Burbuja Azul Pastel) --}}
                    <div class="mb-3">
                        <label for="id_ninio" class="form-label fw-bold text-dark opacity-75 small">Seleccionar Niño</label>
                    ...
                    {{-- Input: Fecha (Burbuja Verde Pastel) --}}
                    <div class="mb-4">
                        <label for="fecha" class="form-label fw-bold text-dark opacity-75 small">Fecha del Consumo</label>
                        <div class="input-group align-items-center">
...
<script>
    document.getElementById('formCreateRegistroComida').addEventListener('submit', function(e) {
        e.preventDefault(); // Pausar envío nativo automático para procesar SweetAlert2
        ...
        // 1. COMPROBACIÓN LÓGICA: Validar que los select tengan opciones seleccionadas
        if (ninioSelect.value === "" || platoSelect.value === "" || !fechaInput) {
            swalWithBootstrapButtons.fire({
                title: "Campos Incompletos",
                text: "Por favor, completa la selección de todos los campos requeridos.",
                icon: "warning",
        ...
        // Formatear fecha para el cuadro de diálogo (DD/MM/YYYY)
        const partesFecha = fechaInput.split('-');
        const fechaFormateada = `${partesFecha[2]}/${partesFecha[1]}/${partesFecha[0]}`;

        // 2. ADVERTENCIA E INTERACCIÓN CONFIRMACIÓN FINAL
        swalWithBootstrapButtons.fire({
            title: "¿Guardar registro de comida?",
            text: `Vas a registrar que el alumno "${textoNinio}" consumió el platillo "${textoPlato}" en la fecha ${fechaFormateada}.`,
            icon: "question",
            ...
            if (result.isConfirmed) {
                this.submit(); // Completar envío si el administrador da luz verde
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                swalWithBootstrapButtons.fire({
                    title: "Registro Cancelado",
                    text: "No se ha añadido ningún elemento al historial.",
                    icon: "info",
