        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white">
            <div class="card-body p-4">
                ...
                <div class="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom border-light">
                    ...
                    <div>
                        <h5 class="fw-bold text-dark mb-0" style="letter-spacing: -0.5px;">Registrar Nueva Alergia</h5>
                        <small class="text-muted">Añade una restricción alimentaria al expediente del alumno</small>
                    </div>
                </div>

                <form action="{{ route('alergias.store') }}" method="post" id="formCreateAlergia">
                    @csrf
                    ...
                    {{-- Select: Ingrediente Alérgeno (Burbuja Pink Pastel de Advertencia) --}}
                    <div class="mb-4">
                        ...
                        <div class="input-group align-items-center">
                            ...
                            <select class="form-select bg-light border-0 rounded-end-3 py-2 text-dark fw-medium" name="id_ingrediente" id="id_ingrediente" style="font-size: 14px; height: 42px;" required>
                                <option value="" selected disabled>Elija el ingrediente...</option>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2">
                        ...
                        <button type="submit" class="btn btn-primary rounded-3 fw-bold px-4 py-2 shadow-sm" style="background-color: #2e5bff; border: none; font-size: 13px;">
                            <i class="fa-solid fa-floppy-disk me-1"></i> Guardar Alergia
                        </button>
                    </div>
...
<script>
    document.getElementById('formCreateAlergia').addEventListener('submit', function(e) {
        e.preventDefault(); // Detener el envío nativo automático para procesar SweetAlert2

        const ninioSelect = document.getElementById('id_ninio');
        const ingredienteSelect = document.getElementById('id_ingrediente');

        // Configuración estética de los botones basándonos en tus clases estables de Bootstrap
        ...
        // 1. COMPROBACIÓN LÓGICA: Validar que los select tengan opciones reales elegidas
        if (ninioSelect.value === "" || ingredienteSelect.value === "") {
            swalWithBootstrapButtons.fire({
                title: "Campos Incompletos",
                text: "Por favor, selecciona tanto al alumno como el ingrediente alérgeno antes de guardar.",
                icon: "warning",
        ...
        // Limpiar las cadenas para extraer el nombre legible del niño y el ingrediente en tiempo real
        ...
        swalWithBootstrapButtons.fire({
            title: "¿Registrar restricción médica?",
            text: `Vas a marcar de forma oficial que el alumno "${textoNinio}" es alérgico a: "${textoIngrediente}". El comedor bloqueará este ingrediente en sus minutas asignadas.`,
            icon: "warning",
            showCancelButton: true,
            confirmButtonText: "Sí, registrar alerta",
            ...
            if (result.isConfirmed) {
                ...
                swalWithBootstrapButtons.fire({
                    title: "Cancelado",
                    text: "No se ha generado ninguna restricción alimentaria nueva.",
                    icon: "info",
